...
-- Expected: 874 Rows
select first_name, last_name from customer where postal_code not in ('M3H', 'K7R', 'L2V', 'K7S', 'J6A');
...
-- Expected: 5 Rows.
select first_name from customer where phone like '(952)%';
-- Find customers with a '.com' email_address
-- Expected: 599 Rows.
select first_name, last_name from customer where email_address like '%.com';
-- Which customers don't have a phone number?
-- Expected: 68 Rows.
select first_name, last_name from customer where phone = '';
-- Which employees don't have an end_date?
-- In other words, end_date has the null value.
-- Expected: 31 Rows.
select first_name, end_date from employee where end_date is null;
...
select * from project_item order by quantity desc limit 1;
